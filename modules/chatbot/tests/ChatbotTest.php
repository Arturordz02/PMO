...
runTest("Resiliencia: El sistema opera normalmente si modules/chatbot/ no existe", function () use ($projectDir, $moduleDir) {
    ...
    // Probar temporalmente renombrando chatbot-view.php para verificar que la página renderiza normalmente sin error fatal
    $viewFile = $moduleDir . '/chatbot-view.php';
    $tempRename = $moduleDir . '/chatbot-view.php.bak';
    rename($viewFile, $tempRename);

    try {
        $html = renderViewWithChatbot('home');
        if (str_contains($html, 'Fatal error') || str_contains($html, 'Warning')) {
            return "Se produjeron errores PHP al no existir chatbot-view.php";
        }
        ...
        if (file_exists($tempRename)) {
            rename($tempRename, $viewFile);
        }
    }
});
...
runTest("create_production_zip.php incluye modules/chatbot/ en paquete de producción y cierre", function () use ($projectDir) {
    ...
    // Verificar que los assets funcionales del chatbot estén incluidos en producción y cierre
    $chatbotFiles = [
    ...
    foreach ($chatbotFiles as $f) {
        if (\Artisan\PackagingPolicy::shouldExcludeFromProduction($f)) {
            return "El archivo {$f} está erróneamente excluido de producción por PackagingPolicy";
        }
        if (\Artisan\PackagingPolicy::shouldExcludeFromClosure($f)) {
            return "El archivo {$f} está erróneamente excluido de código fuente por PackagingPolicy";
        }
    }

    // Verificar que las pruebas del chatbot estén estrictamente excluidas de producción
    if (!\Artisan\PackagingPolicy::shouldExcludeFromProduction('modules/chatbot/tests/ChatbotTest.php')) {
        return "modules/chatbot/tests/ChatbotTest.php no está excluido del paquete de producción";
    }

    // Verificar que las pruebas del chatbot estén incluidas en código fuente
    if (\Artisan\PackagingPolicy::shouldExcludeFromClosure('modules/chatbot/tests/ChatbotTest.php')) {
        return "modules/chatbot/tests/ChatbotTest.php está erróneamente excluido del paquete de código fuente";
    }
