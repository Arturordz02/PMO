...
class DatabaseSchemaTest {
    ...
    private function splitSqlStatements(string $sql, string $prefix): array {
        ...
        // Prefijar nombres de restricciones CONSTRAINT para evitar colisiones globales en InnoDB (error 1005 / errno 121)
        $sql = preg_replace('/CONSTRAINT\s+`([^`]+)`/i', 'CONSTRAINT `' . $prefix . '$1`', $sql);
        $sql = preg_replace('/CONSTRAINT\s+([a-zA-Z0-9_]+)/i', 'CONSTRAINT ' . $prefix . '$1', $sql);
