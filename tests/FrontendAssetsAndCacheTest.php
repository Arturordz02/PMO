...
class FrontendAssetsAndCacheTest {
    ...
    private function testContentSecurityPolicyUnification(): void {
        ...
        // Verificación estricta: CSP prohíbe scripts inline ('unsafe-inline' ausente en script-src)
        preg_match('/script-src\s+([^;]+)/', $htaccess, $mHtaScriptSrc);
        $htaScriptSrc = $mHtaScriptSrc[1] ?? '';
        $this->assert(!str_contains($htaScriptSrc, "'unsafe-inline'"), "CSP en .htaccess prohíbe scripts inline ('unsafe-inline' ausente en script-src)");

        preg_match('/script-src\s+([^;]+)/', $securityPhp, $mSecScriptSrc);
        $secScriptSrc = $mSecScriptSrc[1] ?? '';
        $this->assert(!str_contains($secScriptSrc, "'unsafe-inline'"), "CSP en Security.php prohíbe scripts inline ('unsafe-inline' ausente en script-src)");

        // Auditoría estática: ninguna vista en app/Views/ contiene scripts inline ejecutables ni event handlers inline (on[a-z]+=)
        $allViewFiles = [];
        ...
        foreach ($allViewFiles as $vFile) {
            $content = (string)file_get_contents($vFile);
            $rel = str_replace($this->rootDir . '/', '', str_replace('\\', '/', $vFile));

            // 1. Buscar etiquetas <script> que no tengan atributo src
            if (preg_match_all('/<script\b(?![^>]*\bsrc=)[^>]*>(.*?)<\/script>/is', $content, $matches)) {
                foreach ($matches[1] as $scriptBody) {
                    if (trim($scriptBody) !== '') {
                        $scriptViolations[] = "{$rel}: contiene <script> inline ejecutable (" . substr(trim($scriptBody), 0, 40) . "...)";
                    }
                }
            }

            // 2. Buscar atributos inline event handlers: onclick=, onsubmit=, onchange=, etc. (on[a-z]+=)
            if (preg_match_all('/\b(on[a-z]+)\s*=/i', $content, $mHandlers)) {
                foreach ($mHandlers[1] as $handler) {
                    $handlerViolations[] = "{$rel}: contiene event handler inline '{$handler}='";
                }
            }
        }

        $this->assert(empty($scriptViolations), "Auditoría CSP: CERO scripts inline ejecutables en app/Views/ (" . (empty($scriptViolations) ? "100% compliant" : implode(', ', $scriptViolations)) . ")");
        $this->assert(empty($handlerViolations), "Auditoría CSP: CERO event handlers inline (on[a-z]+=) en app/Views/ (" . (empty($handlerViolations) ? "100% compliant" : implode(', ', $handlerViolations)) . ")");
    }
