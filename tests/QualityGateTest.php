<?php
/**
 * PMO SOLUTIONS — Suite de Pruebas del Quality Gate & Empaquetador (Tarea 10)
 *
 ...
 *   1. Ejecución exitosa del Quality Gate (Código de salida 0).
 *   2. Generación de reportes JSON y TXT limpios de secretos y PII.
 *   3. Bloqueo obligatorio ante error de sintaxis PHP en copia temporal aislada.
 *   4. Bloqueo obligatorio ante archivo prohibido (.env / .zip) en copia temporal aislada.
 *   5. Bloqueo obligatorio ante ruta absoluta local hardcodeada en copia temporal aislada.
 *   6. Bloqueo obligatorio ante discrepancia de directiva CSP (Apache vs PHP).
 *   7. Manejo tolerante de Node.js o MySQL ausentes (reportados como SKIP, sin falsos PASS).
 *   8. Bloqueo del empaquetador (create_production_zip.php) si el Quality Gate falla.
 *   9. Comportamiento condicional de --skip-quality-gate (permitido solo en development).
 *  10. Exclusión estricta de storage/reports/ del ZIP de producción.
 */
...
echo "\n======================================================================\n";
echo " SUITE DE PRUEBAS: QUALITY GATE & EMPAQUETADO PRE-DESPLIEGUE (TAREA 10)\n";
echo "======================================================================\n";
...
final class QualityGateTest {
    ...
    private function testSuccessfulQualityGateExecution(): void {
        ...
        $phpBin = PHP_BINARY;
        $cmd = "\"{$phpBin}\" \"{$qgScript}\" \"{$this->rootDir}\" 2>&1";
    ...
    private function testPackagerRefusesOnQualityGateFailure(): void {
        ...
        $prodZip = $tempOutDir . DIRECTORY_SEPARATOR . 'PMO-Solutions-Produccion-Tarea9.zip';
        $cierreZip = $tempOutDir . DIRECTORY_SEPARATOR . 'PMO-Solutions-Tarea9-Cierre.zip';
        ...
        $cmd = "\"{$phpBin}\" \"{$packager}\" \"{$tempOutDir}\" 2>&1";
        exec($cmd, $out, $exitCode);

        $this->assert($exitCode !== 0, "create_production_zip.php aborta con código != 0 cuando Quality Gate falla");
        $this->assert(!file_exists($prodZip), "El archivo anterior/parcial PMO-Solutions-Produccion-Tarea9.zip fue ELIMINADO");
        $this->assert(!file_exists($cierreZip), "El archivo anterior/parcial PMO-Solutions-Tarea9-Cierre.zip fue ELIMINADO");
    ...
    private function testSkipQualityGateFlagRestrictions(): void {
        ...
        $cmdProdEnv = "\"{$phpBin}\" \"{$packager}\" \"{$tempOutDir}\" --skip-quality-gate 2>&1";
        ...
        $cmdProdRunner = "\"{$phpBin}\" \"{$packager}\" \"{$tempOutDir}\" 2>&1";
        ...
        $cmdDevEnv = "\"{$phpBin}\" \"{$packager}\" \"{$tempOutDir}\" --skip-quality-gate 2>&1";
    ...
    private function testReportsExcludedFromProductionZip(): void {
        ...
        $cmd = "\"{$phpBin}\" \"{$packager}\" \"{$tempOutDir}\" --skip-quality-gate 2>&1";
        exec($cmd, $out, $code);

        putenv('PMO_APP_ENV');
        unset($_ENV['PMO_APP_ENV']);

        $prodZipPath = $tempOutDir . DIRECTORY_SEPARATOR . 'PMO-Solutions-Produccion-Tarea9.zip';
    ...
    private function printSummary(): void {
        ...
        echo "\n======================================================================\n";
        echo " RESUMEN: PRUEBAS DEL QUALITY GATE (TAREA 10)\n";
        echo "======================================================================\n";
