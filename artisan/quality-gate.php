...
foreach ($deprecatedTokens as $token => $desc) {
    ...
    foreach ($phpFiles as $file) {
        $rel = ltrim(substr($file, strlen($projectDir)), '/');
        // Excluir tests de verificación y scripts artisan de limpieza
        if (str_starts_with($rel, 'tests/') || $rel === 'artisan/quality-gate.php' || $rel === 'artisan/create_production_zip.php') {
            continue;
        }
...
$essentialFiles = [
    ...
    '.htaccess'                                                => 'Apache Front Controller & Hardening',
