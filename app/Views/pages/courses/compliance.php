<!-- 1. Encabezado Hero Estilizado (Corporate Integrity & Compliance Governance Style) -->
  <header class="compliance-hero-section">
    <div class="hero-grid-overlay"></div>
    <div class="container position-relative">
      <div class="row align-items-center gy-5">
        <div class="col-12 col-lg-7">
          <div class="mb-2">
            <span class="badge bg-success text-white px-3 py-2 rounded-pill fw-bold mb-2 hero-badge shadow-sm">
              <i class="fas fa-shield-alt me-2"></i> INTEGRIDAD CORPORATIVA & ISO 37001
            </span>
          </div>
          <h1 class="hero-title mb-2 animate__animated animate__fadeInDown">GESTIÓN DE COMPLIANCE EN LA CONSTRUCCIÓN</h1>
          <p class="hero-subtitle mb-4 animate__animated animate__fadeInUp animate__delay-1s">
            Fortalece la cultura de integridad y gestión de riesgos reputacionales a través de modelos de prevención alineados a la Ley N° 30424, ISO 37001, PLAFT, antifraude, libre competencia y protección de datos.
          </p>
          <div class="hero-actions d-flex flex-wrap gap-2 pt-2">
            <a href="#temario-contenido" class="btn btn-warning btn-lg fw-bold shadow text-white hvr-grow me-md-2 mb-2 mb-md-0">
              <i class="fas fa-book-open me-2"></i> Ver Temario Completo
            </a>
            <a href="#seccion-inscripcion" class="btn btn-outline-light btn-lg fw-bold shadow hvr-grow mb-2 mb-md-0">
              <i class="fas fa-credit-card me-2"></i> Inscripción & Medios de Pago
            </a>
          </div>

          <!-- Feature badges -->
          <div class="d-flex flex-wrap gap-3 gap-md-4 mt-4 mt-md-5 pt-3 border-top border-white border-opacity-10 text-white-50">
            <div class="d-flex align-items-center gap-2">
              <i class="fas fa-balance-scale text-success fs-5"></i>
              <span class="text-white small fw-semibold">Ley N° 30424 & ISO 37001 Antisoborno</span>
            </div>
            <div class="d-flex align-items-center gap-2">
              <i class="fas fa-shield-virus text-warning fs-5"></i>
              <span class="text-white small fw-semibold">Prevención PLAFT & Antifraude</span>
            </div>
            <div class="d-flex align-items-center gap-2">
              <i class="fas fa-briefcase text-info fs-5"></i>
              <span class="text-white small fw-semibold">Casos Reales de Infraestructura</span>
            </div>
          </div>
        </div>

        <div class="col-12 col-lg-5">
          <!-- Grid de 3 tarjetas de valor ejecutivo dentro del Hero -->
          <div class="d-flex flex-column gap-3">
            
            <div class="risk-metric-box d-flex align-items-start gap-3 text-white shadow-sm">
              <div class="p-3 rounded-3 bg-success text-white fs-4">
                <i class="fas fa-user-shield"></i>
              </div>
              <div>
                <h5 class="fw-bold mb-1 text-white">Anticorrupción & Ley 30424</h5>
                <p class="small text-white-50 mb-0">Implementación de modelos de prevención antisoborno, mitigación penal e ISO 37001.</p>
              </div>
            </div>

            <div class="risk-metric-box d-flex align-items-start gap-3 text-white shadow-sm">
              <div class="p-3 rounded-3 bg-warning text-dark fs-4">
                <i class="fas fa-balance-scale"></i>
              </div>
              <div>
                <h5 class="fw-bold mb-1 text-warning">PLAFT & Antifraude</h5>
                <p class="small text-white-50 mb-0">Prevención del lavado de activos, detección de fraudes en compras y debida diligencia de socios.</p>
              </div>
            </div>

            <div class="risk-metric-box d-flex align-items-start gap-3 text-white shadow-sm">
              <div class="p-3 rounded-3 bg-info text-white fs-4">
                <i class="fas fa-file-contract"></i>
              </div>
              <div>
                <h5 class="fw-bold mb-1 text-white">Datos Personales & Competencia</h5>
                <p class="small text-white-50 mb-0">Cumplimiento estricto de la Ley N° 29733 y programas preventivos de Libre Competencia.</p>
              </div>
            </div>

          </div>
        </div>
      </div>
    </div>
  </header>

  <!-- 2. Presentación del Curso (Flyer Clickeable + Objetivo + Resumen + Docente) -->
  <section class="py-5 my-lg-4">
    <div class="container">
      <div class="row g-4 g-lg-5 align-items-center">
        
        <!-- Columna Izquierda: Flyer Clickeable -->
        <div class="col-12 col-lg-5">
          <div data-aos="fade-right" data-aos-delay="100" class="card border-0 shadow-lg rounded-4 overflow-hidden position-relative group-hover hvr-box-shadow-outset">
            <a href="https://docs.google.com/forms/d/e/1FAIpQLSdEFpjxGQkRDyFAMb0oqys9dLDm20etfkj_CGeTWrhpZgFX1w/viewform" target="_blank" rel="noopener noreferrer">
              <img src="img/picture6b_orig.png" alt="Flyer Gestión de Compliance en la Construcción" class="img-fluid w-100 rounded-4 transition-zoom" style="display: block;">
              <div class="card-img-overlay d-flex align-items-end p-3 bg-dark bg-opacity-25 opacity-hover hvr-box-shadow-outset">
                <span class="btn btn-warning btn-sm fw-bold w-100 shadow text-white hvr-grow"><i class="fas fa-external-link-alt me-2"></i> Clic para Inscripción Directa</span>
              </div>
            </a>
          </div>
          <div class="text-center mt-2">
            <small class="text-muted"><i class="fas fa-info-circle me-1"></i> Haz clic en el afiche para solicitar el temario y brochure oficial</small>
          </div>
        </div>

        <!-- Columna Derecha: Objetivo, Resumen & Docente -->
        <div class="col-12 col-lg-7">
          <span class="section-badge">Ética & Gobierno Corporativo</span>
          <h2 class="section-title mb-3">Blindaje Legal y Mitigación de Riesgos Reputacionales</h2>
          
          <!-- Objetivo del Curso (Card Técnica con Borde Verde) -->
          <div class="p-4 rounded-4 bg-light border-start border-4 border-success mb-4 shadow-sm">
            <div class="d-flex align-items-center gap-2 mb-2">
              <i class="fas fa-bullseye text-success fs-5"></i>
              <h5 class="fw-bold text-success mb-0">Objetivo del Curso</h5>
            </div>
            <p class="text-secondary mb-0" style="line-height: 1.7;">
              Fortalecer la cultura de Integridad y Cumplimiento de las organizaciones a través de un entendimiento claro y eficiente de la gestión de riesgos reputacionales por medio de la implementación de la gestión de Compliance alineada con prácticas líderes.
            </p>
          </div>

          <!-- Módulos de Fundamentos -->
          <div class="p-3 rounded-4 bg-white border border-light-subtle shadow-sm mb-4">
            <h6 class="fw-bold text-primary mb-2"><i class="fas fa-balance-scale text-success me-2"></i>Fundamentos de Integridad y Gobernanza:</h6>
            <div class="row g-2">
              <div class="col-12 col-md-6">
                <div class="small text-secondary"><i class="fas fa-check-circle text-success me-1"></i> Marcos de referencia y estructura de gobierno.</div>
              </div>
              <div class="col-12 col-md-6">
                <div class="small text-secondary"><i class="fas fa-check-circle text-success me-1"></i> Gestión de riesgos reputacionales y Debida Diligencia.</div>
              </div>
              <div class="col-12">
                <div class="small text-secondary"><i class="fas fa-check-circle text-success me-1"></i> Modelos de prevención bajo la Ley N° 30424, modificatorias y estándar ISO 37001.</div>
              </div>
            </div>
          </div>

          <!-- Ficha del Docente Principal (Panel Ejecutivo Extendido) -->
          <div class="card bg-light border-0 rounded-4 p-4 mt-4 shadow-sm border-start border-5 border-primary" data-aos="fade-up">
            <div class="row align-items-center g-3">
              <div class="col-md-3 text-center">
                <img src="img/RenzoValera.png" alt="RENZO VALERA" class="rounded-circle shadow border border-3 border-white" style="width: 110px; height: 110px; object-fit: cover; object-position: top;">
              </div>
              <div class="col-md-9">
                <span class="badge bg-primary text-white mb-2 px-3 py-2 rounded-pill"><i class="fas fa-user-tie me-1"></i> DOCENTE PRINCIPAL</span>
                <h4 class="fw-bold mb-1 text-dark">RENZO VALERA</h4>
                <p class="small text-primary fw-bold mb-2">Ex Senior Manager Advisory EY · Ex Gerente Compliance Intercorp</p>
                
                <!-- Resumen Completo y Trayectoria Profesional -->
                <p class="small text-muted mb-2" style="line-height: 1.6;">
                  Contador Público (UNMSM) y Maestría (e) en Auditoría por la Universidad del Pacífico. 20 años de experiencia en Gestión Integral de Riesgos, Control Interno y Compliance. Ex Gerente de Compliance en la Plataforma de Educación de Intercorp y Ex Senior Manager de Advisory en EY (Ernst & Young).
                </p>
                
                <div class="d-flex flex-wrap gap-2 mt-2">
                  <span class="badge bg-white text-dark border"><i class="fas fa-shield-alt text-warning me-1"></i> Auditor Líder Compliance</span>
                  <span class="badge bg-white text-dark border"><i class="fas fa-briefcase text-info me-1"></i> Casuística Real en Obra</span>
                </div>
              </div>
            </div>
          </div>
  </section>

    <!-- GARANTÍA DE APRENDIZAJE & BENEFICIOS CLAVE -->
  <section class="py-5 bg-light">
    <div class="container">
      <div class="text-center mb-4" data-aos="fade-up">
        <span class="section-badge">Garantía de Excelencia</span>
        <h3 class="fw-bold text-primary">¿Qué incluye tu Inscripción en este Programa?</h3>
        <p class="text-secondary small">Diseñado para profesionales que buscan aplicación técnica inmediata en obra y oficina técnica.</p>
      </div>
      <div class="row g-4">
        <div class="col-12 col-md-4" data-aos="fade-right" data-aos-delay="100">
          <div class="card h-100 border-0 shadow-sm rounded-4 p-4 text-center bg-white hvr-box-shadow-outset">
            <div class="p-3 rounded-circle bg-primary bg-opacity-10 text-primary mx-auto mb-3" style="width:60px;height:60px;display:flex;align-items:center;justify-content:center;">
              <i class="fas fa-play-circle fs-3"></i>
            </div>
            <h5 class="fw-bold text-primary mb-2">Clases Grabadas / Virtual Online 24/7</h5>
            <p class="text-muted small mb-0">Aprende a tu propio ritmo con lecciones grabadas en HD y acceso digital permanente de por vida a los contenidos.</p>
          </div>
        </div>
        <div class="col-12 col-md-4" data-aos="fade-up" data-aos-delay="200">
          <div class="card h-100 border-0 shadow-sm rounded-4 p-4 text-center bg-white hvr-box-shadow-outset">
            <div class="p-3 rounded-circle bg-warning bg-opacity-15 text-warning mx-auto mb-3" style="width:60px;height:60px;display:flex;align-items:center;justify-content:center;">
              <i class="fas fa-user-shield fs-3"></i>
            </div>
            <h5 class="fw-bold text-primary mb-2">Modelos de Prevención ISO 37001</h5>
            <p class="text-muted small mb-0">Implementación práctica de matrices de debida diligencia, canales de denuncia y matrices de riesgos penales y antisoborno.</p>
          </div>
        </div>
        <div class="col-12 col-md-4" data-aos="fade-left" data-aos-delay="300">
          <div class="card h-100 border-0 shadow-sm rounded-4 p-4 text-center bg-white hvr-box-shadow-outset">
            <div class="p-3 rounded-circle bg-success bg-opacity-10 text-success mx-auto mb-3" style="width:60px;height:60px;display:flex;align-items:center;justify-content:center;">
              <i class="fas fa-folder-open fs-3"></i>
            </div>
            <h5 class="fw-bold text-primary mb-2">Plantillas & Casos Reales Editables</h5>
            <p class="text-muted small mb-0">Descarga materiales de trabajo, matrices de cálculo en Excel, modelos de informes periciales y contratos de megaproyectos.</p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- PREGUNTAS FRECUENTES DEL PROGRAMA -->
  <section class="py-5 bg-white">
    <div class="container">
      <div class="text-center mb-4" data-aos="fade-up">
        <span class="section-badge">Dudas Frecuentes</span>
        <h3 class="fw-bold text-primary">Preguntas Frecuentes sobre este Curso</h3>
      </div>
      <div class="row justify-content-center">
        <div class="col-12 col-lg-9" data-aos="fade-up" data-aos-delay="100">
          <div class="accordion faq-accordion" id="faqCourseAccordion">
            
            <div class="accordion-item">
              <h3 class="accordion-header" id="faqCourseHead_0">
                <button class="accordion-button " type="button" data-bs-toggle="collapse" data-bs-target="#faqCourseCol_0" aria-expanded="true" aria-controls="faqCourseCol_0">
                  <i class="fas fa-question-circle faq-icon-bullet"></i> ¿Qué estándares internacionales de integridad se estudian?
                </button>
              </h3>
              <div id="faqCourseCol_0" class="accordion-collapse collapse show" aria-labelledby="faqCourseHead_0" data-bs-parent="#faqCourseAccordion">
                <div class="accordion-body">
                  Se desarrollan los marcos de ISO 37001 (Sistemas de Gestión Antisoborno), ISO 37301 (Compliance) y las exigencias de la Ley 30424 sobre responsabilidad administrativa de personas jurídicas.
                </div>
              </div>
            </div>
            <div class="accordion-item">
              <h3 class="accordion-header" id="faqCourseHead_1">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#faqCourseCol_1" aria-expanded="false" aria-controls="faqCourseCol_1">
                  <i class="fas fa-question-circle faq-icon-bullet"></i> ¿Cómo se elabora una Matriz de Riesgos de Compliance en obra?
                </button>
              </h3>
              <div id="faqCourseCol_1" class="accordion-collapse collapse " aria-labelledby="faqCourseHead_1" data-bs-parent="#faqCourseAccordion">
                <div class="accordion-body">
                  Se realizan ejercicios de identificación de riesgos en compras, subcontrataciones, licencias, relación con supervisores y funcionarios públicos.
                </div>
              </div>
            </div>
            <div class="accordion-item">
              <h3 class="accordion-header" id="faqCourseHead_2">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#faqCourseCol_2" aria-expanded="false" aria-controls="faqCourseCol_2">
                  <i class="fas fa-question-circle faq-icon-bullet"></i> ¿A quiénes está dirigido este programa?
                </button>
              </h3>
              <div id="faqCourseCol_2" class="accordion-collapse collapse " aria-labelledby="faqCourseHead_2" data-bs-parent="#faqCourseAccordion">
                <div class="accordion-body">
                  A directores de proyecto, gerentes legales, oficiales de cumplimiento, auditores internos y miembros de directorios de empresas constructoras y consultoras.
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

<!-- 4. Sección de Inscripción e Información (CTA Principal) -->
  <section id="seccion-inscripcion" class="container my-5" data-aos="zoom-in">
    <div class="enrollment-checkout-card">
      
      <!-- Badge Luminoso -->
      <div class="mb-3">
        <span class="badge-inscripciones-glow">
          <span class="pulse-dot"></span>
          <i class="fas fa-rocket"></i> Inscripciones Abiertas 2026
        </span>
      </div>

      <!-- Título & Subtítulo -->
      <h3 class="enrollment-title">GESTIÓN DE COMPLIANCE EN LA CONSTRUCCIÓN</h3>
      <p class="enrollment-subtitle">
        Diseña sistemas antisoborno ISO 37001, matrices de riesgo penal y programas de gobernanza corporativa para obras públicas y privadas.
      </p>

      <!-- Botones de Acción -->
      <div class="d-flex flex-column flex-md-row justify-content-center align-items-center gap-3">

        <a href="https://docs.google.com/forms/d/e/1FAIpQLSdEFpjxGQkRDyFAMb0oqys9dLDm20etfkj_CGeTWrhpZgFX1w/viewform" target="_blank" rel="noopener noreferrer" class="btn enrollment-btn-brochure btn-lg px-4 py-3 shadow w-100 w-md-auto hvr-grow">
          <i class="fas fa-file-pdf me-2"></i> Descargar Brochure & Temario
        </a>

        <a href="https://api.whatsapp.com/send?phone=51944276649&text=Hola%20PMO%20Solutions,%20deseo%20informaci%C3%B3n%20sobre%20el%20curso%20Compliance%20en%20la%20Construcci%C3%B3n" target="_blank" rel="noopener noreferrer" class="btn enrollment-btn-wa btn-lg px-4 py-3 shadow w-100 w-md-auto hvr-grow">
          <i class="fab fa-whatsapp me-2"></i> Consultar por WhatsApp
        </a>

      </div>

      <!-- Franja de Confianza & Seguridad -->
      <div class="enrollment-trust-strip">
        <div class="enrollment-trust-item">
          <i class="fas fa-laptop-code text-warning"></i>
          <span>Clases Grabadas con Acceso Flexible</span>
        </div>
        <div class="enrollment-trust-item">
          <i class="fas fa-university text-info"></i>
          <span>Transferencia Bancaria, Yape y Plin</span>
        </div>
        <div class="enrollment-trust-item">
          <i class="fas fa-file-invoice text-success"></i>
          <span>Facturación Electrónica B2B con RUC</span>
        </div>
      </div>

    </div>
  </section>

  <!-- CURSOS COMPLEMENTARIOS RELACIONADOS -->
  <section class="py-5 bg-light">
    <div class="container">
      <div class="text-center mb-4" data-aos="fade-up">
        <span class="section-badge">Rutas de Especialización</span>
        <h3 class="fw-bold text-primary">Cursos Complementarios Recomendados</h3>
        <p class="text-secondary small">Potencia tu perfil técnico con otros programas de alta demanda en el sector.</p>
      </div>
      <div class="row g-4">
        
        <div class="col-12 col-md-4" data-aos="fade-up" data-aos-delay="100">
          <div class="featured-course-card">
            <div class="featured-course-img-wrap">
              <img src="img/picture5b_orig.png" alt="Gestión del Cambio en Contratos del Estado" loading="lazy">
              <div class="featured-course-overlay"></div>
              <span class="featured-course-category badge bg-primary">Contratación Pública</span>
            </div>
            <div class="featured-course-body">
              <div class="featured-course-title">Gestión del Cambio en Contratos del Estado</div>
              <div class="featured-course-desc">Modificaciones contractuales y adicionales bajo la Ley 30225.</div>
              <a href="contratos-estado" class="btn btn-outline-primary btn-sm fw-bold w-100 hvr-grow">
                <i class="fas fa-graduation-cap me-1"></i> Ver Programa
              </a>
            </div>
          </div>
        </div>
        <div class="col-12 col-md-4" data-aos="fade-up" data-aos-delay="200">
          <div class="featured-course-card">
            <div class="featured-course-img-wrap">
              <img src="img/picture1b_orig.png" alt="Dispute Boards (DAB - JRD)" loading="lazy">
              <div class="featured-course-overlay"></div>
              <span class="featured-course-category badge bg-primary">Dispute Boards</span>
            </div>
            <div class="featured-course-body">
              <div class="featured-course-title">Dispute Boards (DAB - JRD)</div>
              <div class="featured-course-desc">Gobernanza ética y prevención de discrepancias en obra.</div>
              <a href="dab-jrd" class="btn btn-outline-primary btn-sm fw-bold w-100 hvr-grow">
                <i class="fas fa-graduation-cap me-1"></i> Ver Programa
              </a>
            </div>
          </div>
        </div>
        <div class="col-12 col-md-4" data-aos="fade-up" data-aos-delay="300">
          <div class="featured-course-card">
            <div class="featured-course-img-wrap">
              <img src="img/gestionriesgos.png" alt="Gestión Integral de Riesgos (PMI®)" loading="lazy">
              <div class="featured-course-overlay"></div>
              <span class="featured-course-category badge bg-primary">Riesgos PMI®</span>
            </div>
            <div class="featured-course-body">
              <div class="featured-course-title">Gestión Integral de Riesgos (PMI®)</div>
              <div class="featured-course-desc">Enfoque preventivo de riesgos operativos y de gobernanza.</div>
              <a href="riesgos-pmi" class="btn btn-outline-primary btn-sm fw-bold w-100 hvr-grow">
                <i class="fas fa-graduation-cap me-1"></i> Ver Programa
              </a>
            </div>
          </div>
        </div>
      </div>
      <div class="text-center mt-4">
        <a href="capacitaciones" class="btn btn-link text-primary fw-bold text-decoration-none">
          <i class="fas fa-th-large me-1"></i> Explorar todo el Catálogo de Cursos
        </a>
      </div>
    </div>
  </section>

  <!-- 5. Footer Corporativo Oficial (Con Logo Limpio en Colores Reales) -->