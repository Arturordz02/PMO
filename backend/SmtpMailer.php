 ...
 * 
 * Incluye plantillas HTML corporativas responsivas para:
 * 1. Notificación de consulta de contacto (para administración)
 * 2. Notificación de Reclamo/Queja (para administración)
 * 3. Constancia / Hoja de Reclamación Virtual (para el cliente/reclamante)
 */
...
class SmtpMailer {
    ...
    public function __construct(array $smtpConfig, array $appConfig = []) {
     ...
     * @param string $htmlBody Contenido en formato HTML
     * @param string $textBody Contenido alternativo en texto plano
     * @param string $replyToEmail Correo para responder
     * @param string $replyToName Nombre para responder
     * @return bool True si se envió exitosamente, False si ocurrió un error
     */
    public function send(
        ...
        if (!$socket) {
            ...
            // Intentar fallback con mail() nativo de PHP si SMTP falla
        ...
        try {
            ...
            if ($encryption === 'tls') {
                ...
                // Re-enviar EHLO tras establecer túnel seguro TLS
                $this->sendCommand($socket, "EHLO {$heloHost}");
                $this->expectResponse($socket, 250);
            }
            ...
            // Iniciar envío del contenido
            $this->sendCommand($socket, "DATA");
            $this->expectResponse($socket, 354);

            // Construir encabezados y cuerpo MIME
            ...
            $headers[] = "Date: " . date('r');
            $headers[] = "Message-ID: <" . md5(uniqid(microtime(), true)) . "@" . ($heloHost ?: 'pmo-solutions.com') . ">";
            ...
            @fclose($socket);
            // Si la sesión SMTP falló, intentar fallback mail()
            return $this->sendNativeMailFallback($toEmail, $toName, $subject, $htmlBody, $replyToEmail, $fromEmail, $fromName);
        }
    }

    /**
     * Fallback utilizando la función nativa mail() de PHP si SMTP no está disponible
     */
    ...
    /**
     * Envía un comando al socket SMTP
     */
    ...
    /**
     * Lee y valida la respuesta del servidor SMTP
     */
    ...
    // =========================================================================
    // PLANTILLAS DE CORREO CORPORATIVAS PMO SOLUTIONS
    // =========================================================================

    /**
     * Envía la notificación de nueva consulta de contacto al correo corporativo
     * 
     * @param array $data
     * @return bool
     */
...
<body>
  <div class='container'>
    ...
    /**
     * Envía la notificación de Reclamo/Queja a la administración de PMO Solutions
     * 
     * @param array $data
     * @return bool
     */
...
<body>
  <div class='container'>
    ...
    /**
     * Envía la constancia formal de la Hoja de Reclamación al usuario/consumidor
     * 
     * @param array $data
     * @return bool
     */
